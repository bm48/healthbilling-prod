...
export type ProviderSheetUiExportLayout = {
   ...
   *  Date of Service, PT RES, PT Paid, PT Pay Status. Takes precedence over `isMinimal` and
   *  `isCondensed` when true. */
 ...
 *  1=First Name, 2=LI, 3=Ins, 6=DOS, 13=PT RES, 14=PT Paid, 15=PT Pay Status (last three
 *  shift +1 when Visit Type is on). */
...
export function providerSheetUiExportHeaders(layout: ProviderSheetUiExportLayout): string[] {
  const { showVisitTypeColumn, showCopayCoinsuranceColumns, officeStaffView, isProviderView, providerLevel, isCondensed, isMinimal, isFrontOffice } = layout
  ...
  const chooseLayout = (): string[] => {
    ...
    if (showCondenseButton && isFrontOffice) {
      return frontOfficeVisualIndices(showVisitTypeColumn).map((i) => columnTitlesFull[i])
    }
...
export function sheetRowsToUiMatrix(
  ...
  const { showVisitTypeColumn, officeStaffView, isProviderView, providerLevel, isCondensed, isMinimal, isFrontOffice } = layout
  const showCondenseButton = !officeStaffView && !isProviderView
  ...
  return rows.map((row) => {
    ...
    if (showCondenseButton && isFrontOffice) {
      return frontOfficeVisualIndices(showVisitTypeColumn).map((i) => withVisitType[i])
    }
    if (showCondenseButton && isMinimal) {
